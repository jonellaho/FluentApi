using System;
using System.Linq;
using Fluent.InheritanceMapped;
using Fluent.InheritanceMapped.Model;

namespace Fluent.ConsoleApp.Commands
{
	public static class TableInheritanceFluentAPI
	{
		public static void Get()
		{
			Console.WriteLine();
			using (var db = new InheritanceMappedDbContext())
			{
				...
				{
					Console.WriteLine("\t" + customer.GetType().Name + " - Id: " + customer.CustomerId + " - Name: " + customer.FirstName + " " + customer.LastName);
					if (customer is CustomerOnline)
					{
						var onlineCustomer = (CustomerOnline)customer;
						Console.WriteLine("\t - UserName: " + onlineCustomer.UserName + " - Email: " + onlineCustomer.Email);
					}
					else if (customer is CustomerOnsite)
					{
						var onsiteCustomer = (CustomerOnsite)customer;
						Console.WriteLine("\t - Location: " + onsiteCustomer.Location + " - Phone: " + onsiteCustomer.Phone);
					}
					Console.WriteLine();
				}


				var onlineCustomers = db.Customers
					.OfType<CustomerOnline>();

				Console.WriteLine("Online Customers:");
				Console.WriteLine();
				foreach (var onlineCustomer in onlineCustomers)
				{
					Console.WriteLine("\t" + onlineCustomer.FirstName + " " + onlineCustomer.LastName + " - Id: " + onlineCustomer.CustomerId);
					Console.WriteLine("\t - UserName: " + onlineCustomer.UserName + " - Email: " + onlineCustomer.Email);
					Console.WriteLine();
				}


				var onsiteCustomers = db.Customers
					.OfType<CustomerOnsite>();

				Console.WriteLine("Onsite Customers:");
				Console.WriteLine();
				foreach (var onsiteCustomer in onsiteCustomers)
				{
					Console.WriteLine("\t" + onsiteCustomer.FirstName + " " + onsiteCustomer.LastName + " - Id: " + onsiteCustomer.CustomerId);
					Console.WriteLine("\t - Location: " + onsiteCustomer.Location + " - Phone: " + onsiteCustomer.Phone);
					Console.WriteLine();
				}
			}
